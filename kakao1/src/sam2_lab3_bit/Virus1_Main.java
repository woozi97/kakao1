...
public class Virus1_Main {
	...
	//바이러스가 2가 총 5개 있다면 11010 처럼 3개의 1을 갖는 경우를 찾는것
	static int countBits(int n) {//n에 어떤 비트가 넘어옴
		int cnt=0; //갯수카운팅을 위한 변수를 초기화
		while(n>0) {//n이 0보다 크다면 세어야할 비트가 있는것,비트가 1인것이 어딘가 있다
			System.out.println("n="+n);
			if((n&1)==1) {//맨오른쪽 비트 확인 1이라면 1이됨, 1001 = 9
				cnt++;
				System.out.println("중간 cnt="+cnt);
			}
			n=n>>1;//맨오른쪽 비트를 없애고 앞으로 쉬프트 시킴(1인 비트를 모두 셀수 있게 됨) 
		}
		System.out.println("결국 cnt="+cnt);
		return cnt;//비트가 1인 갯수를 세는것
	}
	...
	static int solve() {
		...
		for(int subset = 0; subset <1<<VirusCnt; ++subset) {//총 바이러스 중에서 모든 부분집합을 구함, 2^n승 -1까지
			System.out.println("subset"+subset);
			//모든 부분집합 중에서 바이러스 원소의 갯수가 M인것만 사용
			//countBits는 비트 카운트를 세기 위한것
			if(countBits(subset)==M) {
				...
				while(myqueue.size() != 0) {//queue가 empty가 아니라면 탐색 진행, empty되면 종료
					//실제 방문은 poll 할때 함//BFS는 일단 pop(poll)하고 포문 돌림
					Point curr = myqueue.poll();//queue에서 뽑아낸 것을 curr에 넣음, 젤첨에 들어간게 반환
	...
	public static void main(String[] args) {
		...
		for(int i =0;i<N;++i) {
			for(int j=0;j<N;++j) {
				Arr[i][j]=sc.nextInt();
				//입력된 배열에 바이러스가 있다면 바이러스 위치를 배열에 넣음(x, y 저장)
				if(Arr[i][j]==2) {
					Virus[VirusCnt++] = new Point(i,j,0);//바이러스의 수는 바이러스의 인덱스가 되면서 총 갯수가 되기도함
				}
			}
		}
		System.out.println("VirusCnt="+VirusCnt);
